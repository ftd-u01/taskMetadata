'''
    ...
    of the Human Connectome Project's pseudo-gambling behavioral task.
    ...
    Outputs:
        1. Block-design event files (reflecting which condition constitutes the greater
           proportion of trials in a given block)
		    * win.txt
		    * loss.txt
		    * neut.txt
        2. Event-related design event files
            * win_event.txt
            * loss_event.txt
		3. Descriptive stats: Stats.txt
        4. Sync.txt: time in s of sync slide onset--this should be from run time for the E-Prime script.
        
    --Jeff Phillips, 08/26/2022
...
#					if Proc_Trial[i-1] == "FixationBlockPROC" or Proc_Trial[i-1] == "InitialTR": # edited by JSP on 3/28/2017
					if Proc_Trial[i-1] == "FixationBlockPROC" or Proc_Trial[i-1] == "InitialTR" or Proc_Block[i-1] == "InitialTR":
						QM_Onset_Sec = float(QM_Onset[i])/1000.0 - float(Sync_Val) - 8
						#check block type
